...
const PRODUCTION_BUILDINGS = [
    { id: "workshop",      name: "Atelier",                  description: "Fabrique des pièces: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 15,            gain: 0.15,      count: 0, image: "🛠️", imgPath: "images/rocket/workshop.png",  unlockCondition: () => true,            totalGenerated: 0 },
    ...
    { id: "mine",          name: "Mine stellaire",           description: "Extraction de minerai: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 1100,          gain: 11,        count: 0, image: "⛏️",       unlockCondition: () => score >= 500,      totalGenerated: 0 },
    { id: "solar",         name: "Centrale solaire",         description: "Énergie: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 12000,         gain: 120,       count: 0, image: "☀️",       unlockCondition: () => score >= 6000,     totalGenerated: 0 },
    { id: "lab",           name: "Laboratoire",              description: "Recherche: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 130000,        gain: 1300,      count: 0, image: "🧪",       unlockCondition: () => score >= 65000,    totalGenerated: 0 },
    { id: "foundry",       name: "Fonderie orbitale",        description: "Raffinage: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 1400000,       gain: 14000,     count: 0, image: "🔥",       unlockCondition: () => score >= 700000,   totalGenerated: 0 },
    { id: "station",       name: "Station spatiale",         description: "Logistique: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 20000000,      gain: 200000,    count: 0, image: "🚀",       unlockCondition: () => score >= 10000000, totalGenerated: 0 },
    { id: "nanoforge",     name: "Nanoforge",                description: "Fabrication avancée: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 330000000,     gain: 3300000,   count: 0, image: "⚙️",       unlockCondition: () => score >= 150000000, totalGenerated: 0 },
    { id: "synth",         name: "Synthétiseur de matière",  description: "Matière exotique: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 5100000000,    gain: 51000000,  count: 0, image: "✨",       unlockCondition: () => score >= 2500000000, totalGenerated: 0 },
    { id: "antimatter",   name: "Collecteur d'antimatière",  description: "Ressource ultime: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 75000000000,   gain: 750000000, count: 0, image: "🌀",       unlockCondition: () => score >= 35000000000, totalGenerated: 0 },
    { id: "voidrig",       name: "Foreuse du vide",           description: "Forage du vide: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 1e12,          gain: 1e10,      count: 0, image: "⛏️",       unlockCondition: () => score >= 5e11,       totalGenerated: 0 },
    { id: "quasar",        name: "Moteur à quasar",           description: "Énergie de quasar: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 1.5e13,        gain: 1.5e11,    count: 0, image: "💫",       unlockCondition: () => score >= 7.5e12,    totalGenerated: 0 },
    { id: "nebula",        name: "Raffinerie de nébuleuse",   description: "Raffinage de nébuleuse: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 2e14,          gain: 2e12,      count: 0, image: "🌌",       unlockCondition: () => score >= 1e14,      totalGenerated: 0 },
    { id: "pulsar",        name: "Moulure à pulsar",          description: "Moulure à pulsar: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 3e15,          gain: 3e13,      count: 0, image: "⭐",       unlockCondition: () => score >= 1.5e15,    totalGenerated: 0 },
    { id: "blackhole",     name: "Trous noir industriel",     description: "Trous noir industriel: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 5e16,          gain: 5e14,      count: 0, image: "🕳️",       unlockCondition: () => score >= 2.5e16,    totalGenerated: 0 },
    { id: "darkmatter",    name: "Extracteur de matière sombre", description: "Matière sombre: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 8e17,          gain: 8e15,      count: 0, image: "🌑",       unlockCondition: () => score >= 4e17,      totalGenerated: 0 },
    { id: "wormhole",      name: "Usine à trou de ver",       description: "Trou de ver: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 1.2e19,        gain: 1.2e17,    count: 0, image: "🌀",       unlockCondition: () => score >= 6e18,      totalGenerated: 0 },
    { id: "supernova",     name: "Réacteur à supernova",      description: "Supernova: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 2e20,          gain: 2e18,      count: 0, image: "🌟",       unlockCondition: () => score >= 1e20,      totalGenerated: 0 },
    { id: "bigbang",       name: "Forge cosmique",           description: "Forge cosmique: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 3e21,          gain: 3e19,      count: 0, image: "💥",       unlockCondition: () => score >= 1.5e21,    totalGenerated: 0 },
    { id: "singularity",   name: "Singularité productive",    description: "Singularité: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", baseCost: 5e22,          gain: 5e20,      count: 0, image: "🔮",       unlockCondition: () => score >= 2.5e22,    totalGenerated: 0 }
];
...
const ROCKET_PARTS = [
    ...
    { id: "engines",       name: "Moteurs",        description: "Moteurs principaux: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 500,           image: "🔥",       imgPath: "images/rocket/engines.png",       x: 50,    y: 595, width: 40,  height: 51,  order: 3,  purchased: false },
    { id: "fuel-tank",     name: "Réservoir",     description: "Carburant: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 5000,          image: "⛽",       imgPath: "images/rocket/fuel-tank.png",     x: 50,    y: 537, width: 40,  height: 58,  order: 4,  purchased: false },
    { id: "rocket-body",   name: "Corps",          description: "Structure: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 50000,         image: "🏭",       imgPath: "images/rocket/body.png",          x: 50,    y: 337, width: 40,  height: 200, order: 5,  purchased: false },
    { id: "boosters-left", name: "Boosters Gauche", description: "Propulsion supplémentaire: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 500000,        image: "🚀",       imgPath: "images/rocket/boosters-left.png", x: 45.8,  y: 373, width: 50,  height: 300, order: 6,  purchased: false },
    { id: "boosters-right",name: "Boosters Droit",  description: "Propulsion supplémentaire: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 5000000,        image: "🚀",       imgPath: "images/rocket/boosters-right.png",x: 54.2,  y: 373, width: 50,  height: 300, order: 6,  purchased: false },
    { id: "cockpit",       name: "Cockpit",        description: "Poste de pilotage: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 50000000,       image: "👨‍🚀", imgPath: "images/rocket/cockpit.png",        x: 50,    y: 292, width: 45,  height: 45,  order: 7,  purchased: false },
    { id: "shield",        name: "Bouclier",       description: "Protection: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 500000000,      image: "🛡️",       imgPath: "images/rocket/shield.png",        x: 50,    y: 233, width: 45,  height: 59,  order: 8,  purchased: false },
    { id: "launch-pad",    name: "Pas de tir",     description: "Lancement: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 5000000000,     image: "🚀",       imgPath: "images/rocket/launch-pad.png",    x: 60.2,  y: 205, width: 190, height: 481, order: 9,  purchased: false },
    { id: "astronaut",     name: "Astronaute",    description: "Pilote: +{gain} Parts/s\n% de la production: {percent}%\nTotal généré: {total} Parts", cost: 20000000000,   image: "👩‍🚀", imgPath: "images/rocket/astronaut.png",     x: 40,    y: 635, width: 25,  height: 60,  order: 10, purchased: false }
];
...
const PLANETS = [
    ...
    { id: 'mars', name: 'Mars', emoji: '\u2642', distanceRequired: 3840000, bonusPercent: 2, color: '#ef4444', imgPath: 'images/planets/mars.png' },
    { id: 'neptune', name: 'Neptune', emoji: '\u2645', distanceRequired: 38400000, bonusPercent: 3, color: '#06b6d4', imgPath: 'images/planets/neptune.png' },
    { id: 'pluto', name: 'Pluto', emoji: '\u2646', distanceRequired: 384000000, bonusPercent: 5, color: '#8b5cf6', imgPath: 'images/planets/pluto.png' },
    { id: 'oort-cloud', name: 'Oort Cloud', emoji: '\u2728', distanceRequired: 3840000000, bonusPercent: 8, color: '#f59e0b', imgPath: 'images/planets/oort-cloud.png' },
    { id: 'proxima-centauri', name: 'Proxima Centauri', emoji: '\u2609', distanceRequired: 38400000000, bonusPercent: 12, color: '#10b981', imgPath: 'images/planets/proxima-centauri.png' },
    { id: 'sirius', name: 'Sirius', emoji: '\u2609', distanceRequired: 384000000000, bonusPercent: 15, color: '#3b82f6', imgPath: 'images/planets/sirius.png' },
    { id: 'milky-way-center', name: 'Milky Way Center', emoji: '\uD83C\uDF0C', distanceRequired: 3840000000000, bonusPercent: 20, color: '#fbbf24', imgPath: 'images/planets/milky-way-center.png' },
    { id: 'andromeda', name: 'Andromeda', emoji: '\uD83C\uDF0C', distanceRequired: 38400000000000, bonusPercent: 25, color: '#ec4899', imgPath: 'images/planets/andromeda.png' },
    { id: 'virgo-cluster', name: 'Virgo Cluster', emoji: '\u2728', distanceRequired: 384000000000000, bonusPercent: 30, color: '#a855f7', imgPath: 'images/planets/virgo-cluster.png' }
];
